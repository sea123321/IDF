...
class resnet(_fasterRCNN):
  ...
  def _init_modules(self):
    ...
    resnet = resnet101()
    resnet_branch = resnet101()
    if self.layers == 50:
      resnet = resnet50()
      resnet_branch = resnet50()
    if self.pretrained == True:
      print("Loading pretrained weights from %s" %(self.model_path))
      state_dict = torch.load(self.model_path)
      resnet.load_state_dict({k:v for k,v in state_dict.items() if k in resnet.state_dict()})
      resnet_branch.load_state_dict({k:v for k,v in state_dict.items() if k in resnet_branch.state_dict()})
    ...
    # Build resnet_branch.
    self.RCNN_base1_b = nn.Sequential(resnet_branch.conv1, resnet_branch.bn1,resnet_branch.relu,
      resnet_branch.maxpool,resnet_branch.layer1)
    self.RCNN_base2_b = nn.Sequential(resnet_branch.layer2)
    self.RCNN_base3_b = nn.Sequential(resnet_branch.layer3)

    self.netD_1_b = netD_1_b()
    self.netD_2_b = netD_2_b()
    self.netD_3_b = netD_3_b()

    self.RCNN_top_t = nn.Sequential(resnet_branch.layer4)
    feat_d = 2048   

    self.RCNN_cls_score_t = nn.Linear(feat_d, self.n_classes)
    if self.class_agnostic:
      self.RCNN_bbox_pred_t = nn.Linear(feat_d, 4)
    else:
      self.RCNN_bbox_pred_t = nn.Linear(feat_d, 4 * self.n_classes)
    ...
    for p in self.RCNN_base1_b[0].parameters(): p.requires_grad=False
    for p in self.RCNN_base1_b[1].parameters(): p.requires_grad=False
    ...
    self.RCNN_base1_b.apply(set_bn_fix)
    self.RCNN_base2_b.apply(set_bn_fix)
    self.RCNN_base3_b.apply(set_bn_fix)
    self.RCNN_top_t.apply(set_bn_fix)
  ...
  def train(self, mode=True):
    ...
    if mode:
      ...
      self.RCNN_base1_b.eval()
      self.RCNN_base1_b[4].train()
      self.RCNN_base2_b.train()
      self.RCNN_base3_b.train()
      ...
      self.RCNN_base1_b.apply(set_bn_eval)
      self.RCNN_base2_b.apply(set_bn_eval)
      self.RCNN_base3_b.apply(set_bn_eval)
      self.RCNN_top_t.apply(set_bn_eval)

  def _head_to_tail(self, pool5):
    fc7 = self.RCNN_top(pool5).mean(3).mean(2)
    return fc7
  def _head_to_tail_t(self, pool5):
    fc7 = self.RCNN_top_t(pool5).mean(3).mean(2)
    return fc7
